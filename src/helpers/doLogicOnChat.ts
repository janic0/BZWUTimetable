...
const doLogicOnChat = (chat: Chat, now: Date) => {
			...
			getNextLesson(c, now).then((nextLesson) => {
				if (currentLesson) {
					if (nextLesson) {
						...
						findSpecificLesson(c, "SPO", now).then((a) => {
								if (a) {
									if (a.found) {
										add("Don't forget your turnsack")
									}
								}
						})
						add(
					...
					add("in room " + nextLesson.roomCodes[0]);
					add("with teacher " + (data.teachers as any)[nextLesson.teacherCodes[0]]);
					add(
						"starting in " +
							Math.ceil(
								(new Date(
									...
									now.getUTCMonth(),
									now.getUTCDate(),
									parseInt(nextLesson.startTime[0] + nextLesson.startTime[1]) -
										2,
									parseInt(nextLesson.startTime[2] + nextLesson.startTime[3])
								).getTime() -
									new Date().getTime()) /
